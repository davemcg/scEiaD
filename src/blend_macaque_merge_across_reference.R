...
hs_genes_mf_missing <- joined %>% filter(mf_gene_id %in% mf_genes, !mf_gene_id %in% mf_genes_as_hs, hs_total > min_hs_exp) %>% pull(hs_gene_id)
...
# rebuild cell info.
macaque_cell_info <- colnames(all_cells_mf_hs_matrix ) %>% enframe() %>% 
...
gdata::keep(all_cells_mf_hs_matrix, gene_id_converter, maca_mf_matrix ,maca_hs_matrix, sure = T)
## First, convert mouse ID's to human ids 
homo_hs_matrix <- load_rdata('pipeline_data/clean_quant/Mus_musculus/mm-mus_musculus_full_sparse_matrix.Rdata')
mus_mm_matrix <- load_rdata('pipeline_data/clean_quant/Homo_sapiens/hs-homo_sapiens_full_sparse_matrix.Rdata')
