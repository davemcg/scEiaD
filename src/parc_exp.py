...
class PARC:
    ...
    def run_subPARC(self):
        ...
        if self.perturb:
            ...
            G_sim = self.run_perturbation(G_sim)
    ...
    def run_perturbation(self, graph):
        ...
        edges_to_remove = np.random.choice(list(range(len(graph.es))), size = sub_sample_size, replace = False)
       ...
       ### add noise to edge weights 
        ...
        for i in range(len(graph.es)):     
            if graph.es[i]['weight'] is None:
                ...
                graph.es[i]['weight'] = graph.es[i]['weight'] * np.random.uniform(.6, 1.66)
        return graph
...
# parc1 = PARC(reduction,
#              jac_std_global='median',
#              random_seed=1,
#              num_threads=1,
#              small_pop=0,  # disable small pop
#              knn=30,
#              jac_weighted_edges=True,
...
# # %%
# outdf = pd.DataFrame( {"Barcode": list(reduction.index),'labels' :parc1.labels})

# %%

def run_parc(reduction, res, k, i):
    parc1 = PARC(reduction,
                 jac_std_global='median',
                 random_seed=1,
                 num_threads=1,
    ...
    try:
        ...
        print(f"exp-{str(i)}_resolution-{str(res)}_knn-{str(k)} Failed")
        return None
    outdf = pd.DataFrame( {"Barcode": list(reduction.index),'labels' :parc1.labels})
    outdf.to_csv(f'parc_exp/exp-{str(i)}_resolution-{str(res)}_knn-{str(k)}_.csv.gz', index=False)
    return


def run_exp(reduction_file, dist):
    reduction = pd.read_csv(reduction_file, index_col=0)
    knn_uniform = np.random.randint(15, 100, size = 500)
    Parallel(n_jobs=-1)(
        delayed(run_parc)(reduction.sample(frac=.85),dist, k, i ) 
