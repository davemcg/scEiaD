...
c_HS <- coldata %>% filter(grepl('^HS', ID)) %>% 
  ...
  mutate(Covariate = paste0(Donor, '_', Tissue), 
         Tis = case_when(Tissue == 'Re' ~ 'Retina', 
                         Tissue == 'Ch' ~ 'Choroid'),
         
         Age = NA, integration_group = 'Late', TissueNote = info)
c_Organoid <- coldata %>% filter(!grepl('^HS', ID)) %>% 
  mutate(ID = gsub('_Rt','Rt',ID) %>% 
           gsub('_Lf','Lf',.)) %>% 
  separate(ID, remove = FALSE, sep = '_|-', into = c('Source','Weeks','Batch','Sample','PI','Lib')) %>% 
  mutate(Covariate = Batch, Age = str_extract(Weeks, '\\d+') %>% as.integer() * 7, Tis = 'Organoid', integration_group = 'Early', TissueNote = info)
