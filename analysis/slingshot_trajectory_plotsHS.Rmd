...
load('~/data/scEiaD/n_features-5000__transform-counts__partition-universe__covariate-batch__method-scVIprojectionSO__dims-8__preFilter.scEiaDMA__HS_trajectory.Rdata') #sling and diffPT in one
...
```{r, fig.width=15, fig.height=10}
umap %>%
  filter(is.na(TabulaMurisCellType_predict), organism == org) %>% 
  ...
  geom_scattermore(pointsize =  0, pixels = c(1000,1000), alpha = 0.6) + 
  scale_color_manual(values = c(pals::cols25()[1:23],pals::alphabet()) %>% unname()) + 
...
umap %>%
  ...
  geom_scattermore(pointsize =  0, pixels = c(1000,1000), alpha = 0.6) + 
  scale_color_manual(values = c(pals::cols25()[1:23],pals::alphabet(), pals::kelly()) %>% unname()) + 
  cowplot::theme_cowplot() +
  guides(color = guide_legend(override.aes = list(size = 7, alpha = 1) ) ) 
...
umap  %>% ggplot(aes(x=UMAP_1, y=UMAP_2, color = CellType_predict)) + geom_scattermore(pointsize = 10, pixels = c(1000,1000)) + scale_color_manual(values = c(pals::alphabet(), pals::alphabet2()) %>% unname()) + cowplot::theme_cowplot() + facet_wrap(~Platform)
...
psTime %>% 
  ...
  facet_wrap(~curve) + 
  geom_scattermore(data = umap %>% filter(organism == org), aes(x=UMAP_1, y = UMAP_2), color = 'gray', pointsize = 0, alpha = 0.4) + 
  geom_scattermore(pointsize = 0, alpha = 0.4) + 
  cowplot::theme_cowplot() + 
...
```{r}
select_curves <- seq(1,65,1)#[-c(8,19,20,41,45, 47, 49, 52,56, 65)]
psTime %>% 
  as_tibble(rownames = 'Barcode') %>% 
  pivot_longer(contains('curve'), names_to = 'Curve', values_to = 'Pseudotime') %>% 
  filter(!is.na(Pseudotime)) %>% 
...
for (i in paste0('slingPseudotime_', seq(1,11,1))){
...
# Heatmaps for Cones vs Rods
DCT is a S cone marker
MYL4 is a ML cone marker
```{r, fig.width=5, fig.height=9}
genes <- diffPT_tibble %>% 
  filter(Trajectory == 'slingPseudotime_2') %>% 
  filter(Gene %in% tf$ID) %>% 
  filter(FDR < 0.01)
lm <- lm_maker(make_tidy_time('slingPseudotime_2', genes$Gene), test = 'study_covariate')

#lm <- lm_maker(make_tidy_time('slingPseudotime_2', genes$Gene))
pval_cutoff <- 0.01  / nrow(lm$lm)
genesV <- genes %>% 
  filter(Gene %in% 
           (lm$lm %>% 
              unnest(g_results) %>% 
              group_by(Gene, group) %>% 
              #filter(Gene %in% tf$ID) %>% 
              filter(group >= 8, `p.value` < pval_cutoff) %>% 
              group_by(Gene) %>% 
...
a <- hm_maker('slingPseudotime_1', round_to = 0.1, genes = genes %>% filter(Gene %in% tf$ID, FDR == 0) %>% pull(Gene), onlyShowTF = TRUE, max_pseudotime = 15, column_title = 'Curve 1 (Cones)')
#b <- hm_maker('slingPseudotime_34', round_to = 0.1, genes = genesV, onlyShowTF = TRUE, max_pseudotime = 15, column_title = 'Curve 34 (ML Cones)')
b <- hm_maker('slingPseudotime_2', round_to = 0.1, genes = genes %>% filter(Gene %in% tf$ID, FDR == 0) %>% pull(Gene), onlyShowTF = TRUE, max_pseudotime = 15, column_title = 'Curve 2 (Rods)')
# vertical
...
a <- hm_maker('slingPseudotime_1', round_to = 0.1, genes = genesV, onlyShowTF = TRUE, max_pseudotime = 15, column_title = 'Curve 1 (Cones)')
#b <- hm_maker('slingPseudotime_34', round_to = 0.1, genes = genesV, onlyShowTF = TRUE, max_pseudotime = 15, column_title = 'Curve 34 (ML Cones)')
b <- hm_maker('slingPseudotime_2', round_to = 0.1, genes = genesV, onlyShowTF = TRUE, max_pseudotime = 15, column_title = 'Curve 2 (Rods)')
# vertical
# draw(a %v% b %v% c %v% d %v% e)
draw(a + b ,  
     show_heatmap_legend = c(FALSE, FALSE, FALSE, TRUE))
...
genesV <- lm$lm %>% 
  ...
  group_by(Gene, group) %>% 
  #filter(Gene %in% tf$ID) %>% 
  filter(group >= 6, group <= 9, `p.value` < (0.05/6500)) %>% group_by(Gene) %>% 
  summarise(Count = n(), `p.value` = mean(`p.value`), `r.squared` = mean(`r.squared`)) %>% 
  arrange(-`r.squared`) %>% filter(`r.squared` > 0.05) %>% 
...
a <- hm_maker('slingPseudotime_1', round_to = 0.1, genes = genesV, onlyShowTF = TRUE,  column_title = 'Curve 1 (Cones)')
#b <- hm_maker('slingPseudotime_34', round_to = 0.1, genes = genesV, onlyShowTF = TRUE, max_pseudotime = 15, column_title = 'Curve 34 (ML Cones)')
b <- hm_maker('slingPseudotime_2', round_to = 0.1, genes = genesV, onlyShowTF = TRUE,  column_title = 'Curve 2 (Rods)')
# vertical
# draw(a %v% b %v% c %v% d %v% e)
draw(a + b ,  
     show_heatmap_legend = c(FALSE, FALSE, FALSE, TRUE))
...
# LM with individual gene
```{r, fig.width = 3}
gene <- 'YBX3'
lmX <- lm_maker(make_tidy_time('slingPseudotime_1', c('ENSG00000060138','ENSG00000074800')))
lmX$lm_individual %>% rename(hs_gene_id = Gene) %>% 
  ...
  geom_line(aes(group = interaction(group, study_accession), 
                color = `p value`)) +
  geom_line(color = 'black', size = 2, alpha = 0.5,  data = lmX$lm %>% rename(hs_gene_id = Gene) %>% 
              left_join(gene_id_converter %>% select(hs_gene_id, Gene = hs_gene_name)) %>% 
...
gene <- 'YBX3'
lmX <- lm_maker(make_tidy_time('slingPseudotime_2', c('ENSG00000060138','ENSG00000074800')))
lmX$lm_individual %>% rename(hs_gene_id = Gene) %>% 
  left_join(gene_id_converter %>% select(hs_gene_id, Gene = hs_gene_name)) %>% 
  filter(Gene %in% gene) %>% 
  unnest(g_results) %>% 
...
```{r, fig.width =  12, fig.height=12}
tidyPT_145 <- list()
for (i in paste0('slingPseudotime_', c(4,31))){
  tidyPT_145[[i]] <- make_tidy_time(i, genes = genesV, rounding_digits = 0.5)
}

tidyPT_145 %>% bind_rows(.id = 'Curve') %>% filter(Gene %in% genesV, Curve %in% c('slingPseudotime_4','slingPseudotime_31')) %>% left_join(gene_id_converter %>% select(Gene = hs_gene_id, hs_gene_name)) %>% ggplot(aes(x=PT, y=CPM, color = Curve)) + geom_smooth()  + facet_wrap(~hs_gene_name) + cowplot::theme_cowplot()
```
...
m_smoo <- hm_maker(sP,genes = genesV, onlyShowTF = TRUE, output_smooth = TRUE)
hm_plot <- hm_maker(sP, genes = genesV, onlyShowTF = TRUE)
area_data <- m_smoo %>% 
...
lmCalc$slingPseudotime_1$lm %>% rename(hs_gene_id = Gene) %>% 
