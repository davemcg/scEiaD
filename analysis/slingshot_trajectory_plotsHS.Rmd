...
node_pos <- umap_cut %>%
  ...
  summarise(x=mean(UMAP_1), y = mean(UMAP_2) * -1) %>% 
...
umap %>%
  ...
  geom_scattermore(pointsize =  1, pixels = c(1000,1000), alpha = 0.6) + 
...
psTime_data <- psTime %>% 
  ...
  filter(!is.na(Pseudotime)) %>% 
  left_join(umap) %>% 
  #mutate(CellType = gsub('\\d+: ','', CellType_predict)) %>% 
...
select_curves <- seq(1,41,1)#[-c(8,19,20,41,45, 47, 49, 52,56, 65)]
psTime %>% 
  as_tibble(rownames = 'Barcode') %>% 
  pivot_longer(contains('curve'), names_to = 'Curve', values_to = 'Pseudotime') %>% 
  filter(!is.na(Pseudotime)) %>% 
...
for (i in paste0('slingPseudotime_', select_curves)){
  try({draw(hm_maker(i, 15, onlyShowTF = TRUE, round_to = 0.1, column_title = i,  max_val = 3) )})
}
```
...
genes <- diffPT_tibble %>% 
  filter(Trajectory == 'slingPseudotime_6') %>% 
  filter(Gene %in% tf$ID) %>% 
  filter(FDR == 0)
lm <- lm_maker(make_tidy_time('slingPseudotime_6', genes$Gene))
...
genesV <- genes %>% 
  filter(Gene %in% 
           (lm$lm %>% 
              unnest(g_results) %>% 
              group_by(Gene, group) %>% 
              filter(group >= 5, `p.value` < pval_cutoff) %>% 
              group_by(Gene) %>% 
              summarise(Count = n()) %>% 
              arrange(-Count) %>% 
              head(20) %>% 
...
genesW <- genes %>% 
  filter(Gene %in%
           (lm$lm %>% 
              unnest(t_results) %>% 
              filter(term == 'PT', estimate > 0, group >= 5) %>% 
              arrange(`p.value`) %>% 
              head(30) %>% 
              pull(Gene))) %>% 
  pull(Gene)

a <- hm_maker('slingPseudotime_6', round_to = 0.1, genes = genesW, onlyShowTF = TRUE, max_val = 3, max_pseudotime = 11, column_title = 'Curve 6 (Cones)')
#b <- hm_maker('slingPseudotime_34', round_to = 0.1, genes = genesV, onlyShowTF = TRUE, max_pseudotime = 15, column_title = 'Curve 34 (ML Cones)')
b <- hm_maker('slingPseudotime_1', round_to = 0.1, genes = genesW, onlyShowTF = TRUE, max_val = 3, max_pseudotime = 11, column_title = 'Curve 1 (Rods)')
