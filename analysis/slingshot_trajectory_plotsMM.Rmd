...
load('~/data/massive_integrated_eye_scRNA/slingMM_2020_12_29.Rdata')
load('~/data/massive_integrated_eye_scRNA/slingMM_2020_12_29.diff.Rdata')
...
```{r}
select_curves <- c(1,2,3,4,6,7,9,11,12,14,15,16,17,19,23,24,25,29,30,31)
psTime %>% 
  as_tibble(rownames = 'Barcode') %>% 
  pivot_longer(contains('curve'), names_to = 'Curve', values_to = 'Pseudotime') %>% 
  filter(!is.na(Pseudotime)) %>% 
...
# heatmaps for 1,5,16 with matched set of genes
```{r, fig.width=15, fig.height=6}
genes <- diffPT_tibble %>% 
  filter(Trajectory == 'slingPseudotime_5') %>% 
  filter(Gene %in% tf$ID) %>% 
  filter(FDR == 0)
lm <- lm_maker(make_tidy_time('slingPseudotime_5', genes$Gene))
...
genesV <- genes %>% 
  filter(Gene %in% 
           (lm$lm_covariate %>% 
              ...
              pull(Gene))) %>% 
  pull(Gene)
a <- hm_maker('slingPseudotime_1', round_to = 0.1, genes = genesV, onlyShowTF = TRUE, max_pseudotime = 15, column_title = 'Curve 1 (Rods)')
b <- hm_maker('slingPseudotime_5', round_to = 0.1, genes = genesV, onlyShowTF = TRUE, max_pseudotime = 15, column_title = 'Curve 5 (BC)')
c <- hm_maker('slingPseudotime_6', round_to = 0.1, genes = genesV, onlyShowTF = TRUE, max_pseudotime = 15, column_title = 'Curve 16 (Cones)')
# vertical
...
```{r, fig.width =  12, fig.height=12}
tidyPT_145 <- list()
for (i in paste0('slingPseudotime_', c(1,5,16))){
  tidyPT_145[[i]] <- make_tidy_time(i, genes = genesV, rounding_digits = 0.5)
}

tidyPT_145 %>% bind_rows(.id = 'Curve') %>% filter(Gene %in% genesV, Curve %in% c('slingPseudotime_1','slingPseudotime_5','slingPseudotime_16')) %>% left_join(gene_id_converter %>% select(Gene = hs_gene_id, hs_gene_name)) %>% ggplot(aes(x=PT, y=CPM, color = Curve)) + geom_smooth()  + facet_wrap(~hs_gene_name) + cowplot::theme_cowplot()
```
