...
zlmCond <- zlm(~seurat_cluster_fac+0, sca)
save(zlmCond,sca,  file = 'testing/MAST_de_obj.Rdata')
levels(seurat_cluster_fac)
```
^ this is running(slow af), but lets figure out how we wanna do it ourselves 
...
terminal_cluster_mapping <- cluster_similarity %>% filter(max_jac > .8) %>% 
