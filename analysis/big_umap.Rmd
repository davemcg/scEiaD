...
umap %>% 
    ...
    filter(!is.na(CellType), 
           ...
           !CellType %in% c('Doublet', 'Doublets', 'Fibroblasts', 'Red Blood Cells'),
           !grepl('RPE|Vascul', CellType)) %>% 
    mutate(Group = case_when(seurat_clusters %in% c(10,43,33) ~ 'BC1 (10, 43, 33)',
                             seurat_clusters %in% c(27, 97) ~ 'BC2 (27, 97)')) %>% gather(Gene, Expression, RHO:SCGN) %>% 
  filter(!is.na(Group), Gene %in% c('CABP5', 'VSX2', 'PRKCA')) %>% 
  ggplot() + geom_violin(aes(x=Group, y = Expression, colour = Gene))
