...
post <- stats %>% rename(`Post QC Count` = value) %>% 
...
table01 <- cell_info_labels %>% 
  ...
  select(Citation:Count, `Post QC Count`, Labels)

table01$`Post QC Count`[is.na(table01$`Post QC Count`)] <- 0
formattable_01 <- table01 %>% 
  formattable(., list(Count = normalize_bar("lightblue"),
                      `Post QC Count` = normalize_bar("lightblue"),
...
`r sum(table01$"Post QC Count")` cells after QC.
