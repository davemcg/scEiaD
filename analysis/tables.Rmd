...
pre <- cell_info_labels %>% 
  ...
  mutate(Citation = case_when(is.na(Citation) ~ '',
         ...
         PMID = case_when(is.na(PMID) ~ '',
                          TRUE ~ PMID)) %>% 
  group_by(Citation, PMID, study_accession, sample_accession, organism, Platform) %>% 
  summarise(Count = n()) 

post <- umap %>% 
  filter(!is.na(sample_accession)) %>% 
  group_by(study_accession, sample_accession) %>% 
  summarise(`Post QC Count` = n())

pre %>% 
  ungroup() %>% 
  group_by(study_accession, organism, Platform) %>% 
  summarise(Count = sum(Count)) %>% 
  left_join(., stats %>% rename(`Post QC Count` = value) %>% 
              select(study_accession, Platform, `Post QC Count`) %>% 
              group_by(study_accession, Platform) %>% 
              summarise(`Post QC Count` = sum(`Post QC Count`))) %>% 
  arrange(organism, -`Post QC Count`)
```
# Session Info
```{r}
