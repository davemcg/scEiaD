...
load('~/data/massive_integrated_eye_scRNA/Mus_musculus_Macaca_fascicularis_Homo_sapiens__0__n_features5000__counts__universe__batch__scVIprojectionSO__dims10__mindist0.1__nneighbors500__knn0.6__trajHomo_sapiens.Rdata')
load('~/data/massive_integrated_eye_scRNA/trajDiffHS_5000_10.Rdata')
...
umap %>%
  filter(!is.na(TabulaMurisCellType_predict)) %>% 
  mutate(CellType_predict = case_when(is.na(TabulaMurisCellType_predict) ~ 'Unknown', TRUE ~ TabulaMurisCellType_predict)) %>% 
  ggplot(aes(x=UMAP_1, y=UMAP_2, color = TabulaMurisCellType_predict)) + 
  geom_scattermore(pointsize =  0, pixels = c(1000,1000), alpha = 0.6) + 
  scale_color_manual(values = c(pals::watlington(), pals::alphabet(), pals::tableau20()) %>% unname()) + 
  cowplot::theme_cowplot() +
  guides(color = guide_legend(override.aes = list(size = 7, alpha = 1) ) ) 
...
genesV <- genes %>% 
  filter(Gene %in% 
           (lm$lm_covariate %>% 
              ...
              pull(Gene))) %>% 
  pull(Gene)
a <- hm_maker('slingPseudotime_1', round_to = 0.1, genes = genesV, onlyShowTF = TRUE, max_pseudotime = 10, column_title = 'Curve 1 (Rods)')
b <- hm_maker('slingPseudotime_2', round_to = 0.1, genes = genesV, onlyShowTF = TRUE, max_pseudotime = 10, column_title = 'Curve 2 (Rods)')
c <- hm_maker('slingPseudotime_4', round_to = 0.1, genes = genesV, onlyShowTF = TRUE, max_pseudotime = 10, column_title = 'Curve 4 (Rods)')
# vertical
...
```{r, fig.width =  12, fig.height=12}
tidyPT_145 <- list()
for (i in paste0('slingPseudotime_', c(1,4,5))){
  tidyPT_145[[i]] <- make_tidy_time(i, genes = genesV, rounding_digits = 0.5)
}

tidyPT_145 %>% bind_rows(.id = 'Curve') %>% filter(Gene %in% genesV, Curve %in% c('slingPseudotime_1','slingPseudotime_4','slingPseudotime_5')) %>% left_join(gene_id_converter %>% select(Gene = hs_gene_id, hs_gene_name)) %>% ggplot(aes(x=PT, y=CPM, color = Curve)) + geom_smooth()  + facet_wrap(~hs_gene_name) + cowplot::theme_cowplot()
```
