...
load('~/data/scEiaD_v2/merged_xgboost_2021-09-20.Rdata')
load('~/data/scEiaD_v2/merged_stats_2021-09-20.Rdata')
library(tidyverse)
library(cowplot)
accuracy <- accuracy %>% 
  filter(score != 0) %>% 
...
purity <- 1; balance <- 3; mixing <- 3
partition = 'universe'
method = 'scvIprojection'
perf %>%  
  filter(partition %in% c('universe','universeHVG')) %>% 
  select(-clusterN, -clusterMedian, -subset) %>% 
  pivot_wider(names_from = c('Score','Group'), values_from = Value) %>% 
  left_join(accuracy %>% 
              filter(partition %in% c('universe','universeHVG')) %>% 
              group_by(norm, nf, dims, method, knn, epochs) %>% summarise(xgboost_score = mean(score))) %>% 
  mutate(sumZScale =
