...
```{r}
perf_tabula %>% 
  filter(Score == 'LISI', nf == 2000, dims %in% c(8,10,30), knn == 7) %>%
...
```{r}
perf_tabula %>% 
  filter(Score == 'ARI', Group == 'CellType-Cluster', nf == 2000, dims %in% c(8,10, 30), knn == 7) %>%
...
```{r}
perf_tabula %>% filter(Score == 'Silhouette', nf == 2000, dims %in% c(8,10,30), knn == 7) %>% 
    pivot_wider(values_from = Value, names_from = c('Group')) %>% 
  ggplot(aes(y=-Batch, x=Cluster, shape = normalization)) + geom_point(aes(color=method), size = 5) +
  cowplot::theme_cowplot() + scale_color_manual(values = pals::alphabet() %>% unname())
```
...
zscore_tabula <- perf_tabula %>% 
  ...
  #filter(clusterMedian < 6000) 
  
zscore_tabula  %>% 
  arrange(-sumZScale) %>% 
  ggplot(aes(x=method, y = sumZScale, shape = normalization)) + geom_point(aes(color=method), size = 5) +
  cowplot::theme_cowplot() + scale_color_manual(values = pals::alphabet() %>% unname()) + coord_flip()
```
