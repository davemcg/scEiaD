...
library(tidyverse)
library(cowplot)
load('/Volumes/data/projects/nei/mcgaughey/massive_integrated_eye_scRNA/metrics3.Rdata')
```
...
tab_overall <- left_join(celltype, org) %>% 
  ...
  mutate(celltype_sum = celltype_rank + ARIRank + LISIRank + SilhouetteRank,
         ...
         sum = org_rank + LISIRank + ARIRank + SilhouetteRank,
         syn = LISIRank + ARIRank + SilhouetteRank) %>% 
  arrange(sum)
...
perf_bench <- 
  bind_rows(perf_one %>% filter(normalization != 'counts'), 
            perf_one %>% filter(normalization == 'counts', dims == 200, nf == 2000, method == 'scVI'))
umap_bench <- 
  bind_rows(umap_one %>% filter(normalization != 'counts'), 
            umap_one %>% filter(normalization == 'counts', dims == 200, nfeatures == 2000, nneighbors == 100, mindist == 0.3,  method == 'scVI')) %>% 
  filter(knn == 5)
