...
tab_overall <- left_join(celltype, org) %>% 
  ...
  ungroup() %>% 
  mutate(celltype_sum = celltype_rank + ARIRank + LISIRank + SilhouetteRank,
         org_sum = org_rank + LISIRank + ARIRank + SilhouetteRank,
         sum = celltype_rank + study_rank + LISIRank + ARIRank + SilhouetteRank) %>% 
  arrange(sum)
...
umap_bench <- 
  bind_rows(umap_one %>% filter(normalization != 'counts'), 
            umap_one %>% filter(normalization == 'counts', dims == 30, nfeatures == 2000, nneighbors == 30, mindist == 0.3,  method == 'scVI')) %>% 
  filter(knn == 5)
...
tab_bench <- left_join(celltype, org) %>% 
  ...
  select(-contains("median")) %>% 
  ungroup() %>% 
  mutate(synthetic = (ARIRank + LISIRank + SilhouetteRank) / 3)
